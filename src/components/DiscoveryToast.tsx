...
export function DiscoveryToast() {
  ...
  const big = item ? rarityRank(item.rarity) >= 3 : false // epic+
    ...
    <div className="fixed top-24 sm:top-16 right-4 z-50 pointer-events-none">
      <AnimatePresence>
        {item && rarity && (
          <motion.div
            key={item.id}
            initial={{ opacity: 0, y: -16, scale: big ? 0.8 : 0.9 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: -16, scale: 0.9 }}
            transition={big ? { type: 'spring', stiffness: 380, damping: 16 } : { duration: 0.25 }}
